...
function init(): void {
    ...
    $ui.register((ctx: UIContext): void => {
        ...
        function tryGet(eps: string[], i: number, path: string): Promise<FetchResponse> {
            ...
            return ctx.fetch(url, { headers: hdrs() }).then(
                (r) => {
                    ...
                    if (r && r.status === 401 && pref("accessToken", "")) {
                        ...
                        return ctx.fetch(url, { headers: hdrsNoAuth() }).then((r2) => {
        ...
        function trySearch(eps: string[], i: number, body: string): Promise<BgmSearchResponse> {
            if (i >= eps.length) return Promise.reject(new Error("所有 Bangumi 端点均无法连接"))
                ...
                (r) => {
                    ...
                    if (r && r.status === 401 && pref("accessToken", "")) {
                        ...
                        return ctx.fetch(url, { method: "POST", headers: h2, body: body }).then((r2) => {
                            ...
                            lastGoodEndpoint = eps[i]
                            const d2 = r2 ? r2.json<BgmSearchResponse>() : null
                            return d2 || { data: [] }
                    ...
                    lastGoodEndpoint = eps[i]
                    const d = r ? r.json<BgmSearchResponse>() : null
                    return d || { data: [] }
                },
                () => trySearch(eps, i + 1, body)
            )
        ...
        function getEntryMeta(id: number): Promise<EntryMeta> {
            const fallback: EntryMeta = { titles: [], cover: "", score: 0, year: 0 }
        ...
        function fetchSubject(mediaId: number, sid: number, seq: number, bound: boolean): void {
            ...
            apiGet("/v0/subjects/" + sid).then((r) => {
                ...
                const d = r ? r.json<BgmSubject>() : null
                if (!d || !d.id) {
        ...
        function loadExtras(sid: number, seq: number): void {
            ...
            const pEps: Promise<any> = wantEps
                ? apiGet("/v0/episodes?subject_id=" + sid + "&type=0&limit=100&offset=0").then((r) => r ? r.json() : null, () => null)
                : Promise.resolve(null)
            ...
            Promise.all([pChars, pEps, pRels]).then((arr) => {
                ...
                const epsData = arr[1] && arr[1].data && arr[1].data.length ? (arr[1].data as BgmEpisode[]) : null
        ...
        function loadCollection(sid: number, seq: number): void {
            ...
            const pUser: Promise<BgmMe | null> = cachedUser
                ? Promise.resolve(JSON.parse(String(cachedUser)) as BgmMe)
                : apiGet("/v0/me").then((r) => {
